...
**旋转:⏸ pending(主动挂起,2026-06-11)**——90° 步进 + 任意角内接矩形(全计划最高风险点,
见实施计划 P5)。决策:先跑通核心链路(P7 变速 → P8 完整导出 → P9 GIF),之后再评估是否做、
做到什么程度(可能只做 90° 步进,任意角视产品需要取舍)。
...
**目标**:在预览上拖动,平移裁剪窗口、**不露黑边**,预览 = 导出(实施计划 P6)。

**已实现**
- `EditState` 增 `offsetX/offsetY`:裁剪窗口中心的 NDC 偏移(GL 约定 y 朝上,0 = 居中)。
- 纯函数 `clampedCropCenter`(`CropGeometry.kt`):偏移夹到 `|c| ≤ 1 − half`,任何比例 × 缩放 × 偏移组合窗口都不出内容;`withClampedOffsets()` 在比例/缩放变化后夹回合法域**并回写**(消除「空拖死区」,计划步骤 4)。
- `cropEffect` 折进偏移:`Crop(cx±halfW, cy±halfH)`,预览/导出同一真值。
- 预览手势:**单指拖动 + 双指缩放**统一走 `detectTransformGestures`;跟手换算用**内容像素尺寸**(cover 后的 `videoW×s` ↔ NDC 跨度 2,计划步骤 3 的坑);NDC y 朝上 ↔ 屏幕 y 朝下取反。`graphicsLayer` 平移把窗口中心拉到外框中心。
- 双击复位:缩放 + 位置一起归零;切比例 chip 时重夹偏移。
- 单测(`CropGeometryTest.kt`,共 11 例):居中不动、满窗合法域退化 {0}、裁剪轴可拖/未裁轴恒 0、源 × 比例 × 缩放 × 极端偏移笛卡儿积断言窗口 ⊆ [-1,1]、缩小后回写值落新边界且幂等。

**验收(DoD)**
- ✅ 几何不越界由单测覆盖。
- ⏳ 真机:放大后任意方向拖动**跟手**且**不露黑边**;拖到角再缩小无死区;拖动后导出与预览一致。

---

## P7 变速

**目标**:预览倍速与导出共用同一个 `state.speed`;变速是时间轴变换,**绝不进 `buildVideoEffects`**(§5.4)。

**已实现**
- `EditState.speed`(>0):滑竿 0.5×–2× 连续可选,**0.05 步进取整**(1× 可精确选中、去浮点尾数);数值文字按 `"9.99×"` 实测**定宽**(文字变宽会挤压 weight 滑竿轨道 → thumb 重算 → 抖,定宽根治)。
- 预览路:`COMMAND_SET_SPEED_AND_PITCH` 检查后 `setPlaybackSpeed`;音频本静音,与无音轨导出一致。
- 导出路(P8 预支,harness 保持 WYSIWYG):`constantSpeedProvider`(恒速、无变化点,§5.4 配方)接 `EditedMediaItem#setSpeed`;**1× 不设置**,默认路径与此前完全一致。
- 非 1× 时界面显示「输出 ≈ 选取时长 ÷ speed」,供真机对时间轴。
- 留待 P8/P11:`setFrameRate(maxOutputFps)`(speed>1 防输出帧率爆高,值待 §10.6 标定);成品最小时长 `÷speed ≥ 500ms` 语义(§10.6)。

**验收(DoD)**
- ⏳ 真机:拖滑竿播放快慢变化明显、比例/缩放/拖动不受影响;2× 导出时长 ≈ 选取时长一半(抽帧时间映射 P8 对拍时一起验)。
